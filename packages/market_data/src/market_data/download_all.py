...
Usage:
  python -m market_data.download_all
  python -m market_data.download_all --only binance
  python -m market_data.download_all --only dukascopy,yahoo,btcd
"""
...
from market_data.fetch_binance_vision import fetch_klines_vision
from market_data.fetch_btcd import (
    IMPORT_DIR,
    fetch_coingecko_btcd_daily,
    ingest_import_dir,
)
from market_data.fetch_crypto_macro import (
    STABLECOIN_SYMBOLS,
    build_dominance_pct,
...
from market_data.fetch_dukascopy import fetch_dukascopy
from market_data.fetch_fred import fetch_fred_daily
from market_data.fetch_yahoo import fetch_yahoo
    ...
    *,
    ...
    try:
        if want("TOTAL_MCAP"):
            ...
            from market_data.fetch_btcd import ingest_tradingview_csv

            total_dir = candles_root() / "imports" / "total_mcap"
    ...
    try:
        ...
        if want("TOTAL_MCAP") and (tot_mcap is None or tot_mcap.empty) and btc_mcap is not None:
            ...
            if existing is not None and not existing.empty:
                ...
                from market_data.fetch_crypto_macro import _close_only
